...
export function ComposeDialog({ open, onClose }: ComposeDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      {/* Sheet */}
      <div className="relative w-full max-w-lg bg-[#000] border border-white/[0.08] rounded-t-2xl sm:rounded-2xl animate-scale-in">
        {/* Header */}
        ...
        <div className="flex gap-3 p-4">
          ...
          <div className="flex-1 min-w-0">
            <textarea
              ...
              className="w-full bg-transparent text-[#e8f0dc] text-[18px] placeholder-[#536471] resize-none outline-none min-h-[120px] leading-relaxed"
              maxLength={maxLen + 50}
              autoFocus
            />
            <div className="border-t border-white/[0.06] pt-3 mt-2 flex items-center justify-between">
              <div className="flex items-center gap-1">
                <button className="p-2 rounded-full hover:bg-[#a3d977]/10 transition-colors">
                  <svg className="w-5 h-5 text-[#a3d977]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8}>
              ...
              {text.length > maxLen * 0.9 && (
                <span className={cn('text-[13px]', overLimit ? 'text-[#f4212e]' : 'text-[#71767b]')}>
                  {remaining}
                </span>
              )}
            </div>
