...
function highlightContent(text?: string) {
  ...
  return parts.map((part, i) => {
    if (part.startsWith('#')) {
      return <span key={i} className="text-[#a3d977] hover:underline cursor-pointer">{part}</span>
    }
    if (part.startsWith('@')) {
      return <span key={i} className="text-[#a3d977] hover:underline cursor-pointer">{part}</span>
    }
...
export function UserPostCard({
  ...
  return (
    <>
      <article className="relative border-b border-white/[0.06] hover:bg-white/[0.02] transition-colors cursor-pointer px-4 py-3">
          ...
          <div className="flex-1 min-w-0" onClick={handleDoubleTap}>
            ...
            <div className="flex items-center gap-1 flex-wrap">
              <button
                ...
                className="font-bold text-[15px] text-[#e8f0dc] hover:underline truncate"
              >
                {post.authorDisplayName || post.authorUsername || 'User'}
              </button>
              {post.authorIsVerified && (
                <svg className="w-[18px] h-[18px] text-[#a3d977] shrink-0" viewBox="0 0 22 22" fill="none">
              ...
              <span className="text-[#71767b] text-[15px]">@{post.authorUsername || 'user'}</span>
              <span className="text-[#71767b]">·</span>
              <span className="text-[#71767b] text-[15px] shrink-0">{timeAgo(post.createdAt)}</span>
            </div>

            {/* Caption */}
            {post.caption && (
              <p className="text-[15px] leading-[20px] text-[#e8f0dc] mt-0.5 whitespace-pre-wrap break-words">
            ...
            {mediaUrls.length > 0 && (
              <div
                className={cn(
                  'mt-3 rounded-2xl overflow-hidden border border-white/[0.08] max-h-[510px]',
            ...
            <div className="flex items-center justify-between mt-3 max-w-[425px] -ml-2">
              ...
              >
                <div className="p-2 rounded-full group-hover:bg-[#a3d977]/10 transition-colors">
                  <svg className="w-[18px] h-[18px] text-[#71767b] group-hover:text-[#a3d977]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8}>
                    <path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </div>
                {commentCount > 0 && (
                  <span className="text-[13px] text-[#71767b] group-hover:text-[#a3d977]">{formatCount(commentCount)}</span>
                )}
              </button>
              ...
              >
                <div className="p-2 rounded-full group-hover:bg-[#00ba7c]/10 transition-colors">
                  <svg
                    className={cn(
                      ...
                      isReposted ? 'text-[#00ba7c]' : 'text-[#71767b] group-hover:text-[#00ba7c]',
                ...
                {repostCount > 0 && (
                  <span className={cn('text-[13px]', isReposted ? 'text-[#00ba7c]' : 'text-[#71767b] group-hover:text-[#00ba7c]')}>
              ...
              >
                <div className="p-2 rounded-full group-hover:bg-[#f91880]/10 transition-colors">
                  {isLiked ? (
                    ...
                    <svg className="w-[18px] h-[18px] text-[#71767b] group-hover:text-[#f91880]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8}>
                      <path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/>
                    </svg>
                  )}
                </div>
                {likeCount > 0 && (
                  <span className={cn('text-[13px]', isLiked ? 'text-[#f91880]' : 'text-[#71767b] group-hover:text-[#f91880]')}>
              ...
              <div className="flex items-center">
                ...
                >
                  <div className="p-2 rounded-full group-hover:bg-[#a3d977]/10 transition-colors">
                    {isBookmarked ? (
                      <svg
                        className={cn('w-[18px] h-[18px] text-[#a3d977]', bookmarkAnim && 'animate-bookmark-pop')}
                        viewBox="0 0 24 24"
                        fill="currentColor"
                        stroke="currentColor"
                        strokeWidth={1.5}
                      ...
                      <svg className="w-[18px] h-[18px] text-[#71767b] group-hover:text-[#a3d977]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round">
