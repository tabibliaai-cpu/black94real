...
export function StorefrontView() {
  ...
  return (
    <div className="min-h-[calc(100vh-53px-50px)] pb-24">
      {/* Cover */}
      <div className="relative h-32 bg-gradient-to-br from-[#a3d977]/30 via-[#0a0a0a] to-[#1a1a2e]">
        <div className="absolute inset-0 bg-black/30" />
        <button
          onClick={() => navigate('store')}
          className="absolute top-3 left-3 z-10 w-9 h-9 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center"
        >
      ...
      <div className="px-4 -mt-10 relative z-10">
        <div className="flex items-end gap-3">
          <div className="w-20 h-20 rounded-2xl bg-[#0a0a0a] border-4 border-black overflow-hidden flex-shrink-0">
            {businessImage ? (
          ...
          <div className="pb-1">
            <div className="flex items-center gap-1.5">
              <h1 className="text-[18px] font-bold text-[#e8f0dc]">{businessName}</h1>
              <svg className="w-5 h-5 text-[#a3d977]" viewBox="0 0 24 24" fill="currentColor">
                <path d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
              </svg>
            </div>
            <p className="text-[13px] text-[#71767b]">{products.length} products</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          <div className="bg-[#0a0a0a] border border-white/[0.06] rounded-xl p-3 text-center">
            <p className="text-[18px] font-bold text-[#e8f0dc]">{products.length}</p>
            <p className="text-[11px] text-[#71767b]">Products</p>
          </div>
          <div className="bg-[#0a0a0a] border border-white/[0.06] rounded-xl p-3 text-center">
            <p className="text-[18px] font-bold text-[#a3d977]">{totalSold}</p>
            <p className="text-[11px] text-[#71767b]">Sales</p>
          </div>
          <div className="bg-[#0a0a0a] border border-white/[0.06] rounded-xl p-3 text-center">
            <p className="text-[18px] font-bold text-amber-400">{avgRating}★</p>
            <p className="text-[11px] text-[#71767b]">Rating</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 px-4">
        <h2 className="text-[15px] font-bold text-[#e8f0dc] mb-3">All Products</h2>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <p className="text-[15px] text-[#71767b]">No products yet</p>
          </div>
        ) : (
        ...
        {hasMore && !loading && (
          <button
            onClick={loadMore}
            className="w-full py-3 mt-4 rounded-full bg-white/[0.06] text-[14px] font-semibold text-[#a3d977] hover:bg-white/[0.1] transition-colors"
          >
            Load More
          </button>
        )}
