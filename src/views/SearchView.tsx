...
export function SearchView() {
  ...
  return (
    <div className="px-4 pt-2">
      {/* Search bar */}
      <div className="sticky top-0 z-10 bg-black pt-2 pb-3 -mx-4 px-4">
        <div className="flex items-center gap-3 bg-white/[0.06] rounded-full px-4 py-2.5 border border-white/[0.08] focus-within:border-[#a3d977]/50 focus-within:bg-white/[0.08] transition-all">
          <svg className="w-5 h-5 text-[#71767b] shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
      ...
      {searched ? (
        <div>
            ...
            {(['people', 'posts'] as const).map((t) => (
              <button
                ...
                className={`flex-1 py-3 text-[15px] font-medium relative capitalize transition-colors ${
                  tab === t ? 'text-[#e8f0dc] font-bold' : 'text-[#71767b]'
                }`}
              >
                {t}
                {tab === t && (
                  <div className="absolute bottom-0 inset-x-6 h-1 bg-[#a3d977] rounded-full" />
                )}
              </button>
            ))}
          </div>
          ...
          {loading ? (
            <div className="py-8 text-center text-[#71767b]">Searching...</div>
          ) : tab === 'people' ? (
            <div>
              {users.length === 0 ? (
                <p className="text-[15px] text-[#71767b] text-center py-8">No users found</p>
              ) : (
                users.map((u) => (
                  <button
                    key={u.uid}
                  ...
                  >
                    ...
                    <div className="text-left min-w-0">
                      <div className="flex items-center gap-1">
                        <span className="font-bold text-[15px] text-[#e8f0dc] truncate">{u.displayName}</span>
                        {u.isVerified && (
                          <svg className="w-[16px] h-[16px] text-[#a3d977] shrink-0" viewBox="0 0 22 22" fill="none">
                      ...
                      <span className="text-[14px] text-[#71767b] truncate">@{u.username}</span>
                      {u.bio && <p className="text-[13px] text-[#71767b] truncate mt-0.5">{u.bio}</p>}
                    </div>
                  </button>
                ))
              )}
            ...
            <div>
              {posts.length === 0 ? (
                <p className="text-[15px] text-[#71767b] text-center py-8">No posts found</p>
              ) : (
                posts.map((p) => (
                  <div key={p.id} className="px-4 py-3 border-b border-white/[0.06]">
                    <p className="text-[15px] text-[#e8f0dc] leading-relaxed">{p.caption}</p>
                    <p className="text-[13px] text-[#71767b] mt-1">by @{p.authorUsername}</p>
                  </div>
                ))
              )}
            </div>
        ...
        /* Trending */
        <div>
          <h3 className="text-xl font-bold text-[#e8f0dc] mb-4">Trends for you</h3>
          {trendingTopics.map((topic, i) => (
            ...
            >
              ...
              <p className="font-bold text-[15px] text-[#e8f0dc] mt-0.5">#{topic.tag}</p>
              <p className="text-[13px] text-[#71767b] mt-0.5">{topic.count} posts</p>
            </button>
          ))}
        </div>
      )}
