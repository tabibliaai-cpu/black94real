  ...
  return (
    <div>
      ...
      <div className="px-4 pt-3 pb-2">
        <div className="flex items-center gap-3 bg-white/[0.06] rounded-full px-4 py-2.5 border border-white/[0.08] focus-within:border-[#a3d977]/50 transition-all">
      ...
      <div className="border-t border-b border-white/[0.06]">
        <div className="flex items-center justify-between px-4 py-2">
          <h3 className="text-xl font-bold text-[#e8f0dc]">Trends for you</h3>
        </div>
        {TRENDING_TOPICS.map((topic, i) => (
          ...
          >
            ...
            <p className="text-[13px] text-[#71767b] mt-0.5">{topic.posts} posts</p>
          </button>
        ))}
      </div>
      ...
      <div className="px-4 py-4">
        <h3 className="text-lg font-bold text-[#e8f0dc] mb-3">Who to follow</h3>
        {SUGGESTED_USERS.map((u) => (
          <div key={u.id} className="flex items-center gap-3 py-2.5">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#1a2a1a] to-[#0a0a0a] flex items-center justify-center text-[14px] text-[#a3d977] font-bold">
              {u.name[0]}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1">
                <span className="font-bold text-[15px] text-[#e8f0dc]">{u.name}</span>
                {u.verified && (
                  <svg className="w-[16px] h-[16px] text-[#a3d977]" viewBox="0 0 22 22" fill="none">
              ...
              <span className="text-[14px] text-[#71767b]">@{u.username}</span>
            </div>
            <button className="px-4 py-1.5 rounded-full bg-[#e8f0dc] text-black text-[14px] font-bold hover:bg-gray-200 transition-colors">
              Follow
            </button>
          </div>
        ))}
