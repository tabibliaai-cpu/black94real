...
function notifIcon(type: string) {
  switch (type) {
    ...
    case 'follow':
      return <svg className="w-[18px] h-[18px] text-[#a3d977]" viewBox="0 0 24 24" fill="currentColor"><path d="M16 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2" strokeLinecap="round" strokeLinejoin="round"/><circle cx="8.5" cy="7" r="4"/><path d="M20 8v6M23 11h-6" strokeLinecap="round" strokeLinejoin="round"/></svg>
    case 'comment':
    ...
    default:
      return <svg className="w-[18px] h-[18px] text-[#a3d977]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01" strokeLinecap="round" strokeLinejoin="round"/></svg>
  }
}
  ...
  return (
    <div>
      <div className="px-4 pt-3 pb-2">
        <h2 className="text-xl font-bold text-[#e8f0dc]">Notifications</h2>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 px-8 text-center">
          ...
          <h3 className="text-lg font-bold text-[#e8f0dc] mb-1">Nothing to see here — yet</h3>
          <p className="text-[15px] text-[#71767b]">Likes, reposts, and follows will show up here.</p>
        </div>
      ) : (
        <div>
          {notifications.map((notif) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[15px] text-[#e8f0dc] leading-relaxed">
                  <span className="font-bold">{notif.actorName}</span>{' '}
                  {notif.message || notif.type}
                  {!notif.read && <span className="inline-block w-2 h-2 rounded-full bg-[#a3d977] ml-1" />}
                </p>
                <p className="text-[13px] text-[#71767b] mt-0.5">{timeAgo(notif.createdAt)}</p>
              </div>
            </button>
          ))}
        </div>
