...
export function CrmDealsView() {
  ...
  return (
    <div className="pb-24">
      {/* Header */}
      <div className="sticky top-0 z-10 bg-black/90 backdrop-blur-md border-b border-white/[0.06] px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-[#e8f0dc]">Deals</h1>
          </div>
          <button
            onClick={() => {
            ...
            <div key={stage} className="shrink-0 w-[260px]">
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className={cn('w-2.5 h-2.5 rounded-full', stage === 'New' ? 'bg-blue-400' : stage === 'Contacted' ? 'bg-yellow-400' : stage === 'Proposal' ? 'bg-purple-400' : stage === 'Negotiation' ? 'bg-orange-400' : stage === 'Won' ? 'bg-[#a3d977]' : 'bg-red-400')} />
                  <h3 className="text-[13px] font-semibold text-[#e8f0dc]">{stage}</h3>
                  <span className="text-[11px] text-[#536471] bg-white/[0.06] px-1.5 py-0.5 rounded-full">{stageDeals.length}</span>
                </div>
                <span className="text-[11px] text-[#a3d977] font-semibold">{formatCurrency(stageValue)}</span>
              </div>

              {/* Cards */}
              <div className="space-y-2">
                {stageDeals.map((deal) => (
                  <div key={deal.id} className="rounded-xl bg-[#0a0a0a] border border-white/[0.06] p-3">
                    <h4 className="text-[14px] font-semibold text-[#e8f0dc] leading-snug">{deal.name}</h4>
                    <p className="text-[16px] font-bold text-[#a3d977] mt-1">{formatCurrency(deal.value)}</p>
                    <p className="text-[12px] text-[#71767b] mt-1.5">{deal.contactName}</p>
                    <p className="text-[11px] text-[#536471] mt-1">{daysSince(deal.createdAt)}d in stage</p>
                    ...
                    <div className="flex gap-1 mt-2.5 pt-2.5 border-t border-white/[0.04] overflow-x-auto no-scrollbar">
                      {STAGES.filter(s => s !== stage).map(s => (
                        <button
                          ...
                          className="px-2 py-0.5 rounded text-[10px] font-semibold bg-white/[0.06] text-[#e8f0dc] hover:bg-white/[0.1] whitespace-nowrap transition-colors"
                        >
                          → {s}
                        </button>
                      ))}
                ...
                {stageDeals.length === 0 && (
                  <div className="rounded-xl border-2 border-dashed border-white/[0.06] p-6 text-center">
                    <p className="text-[12px] text-[#536471]">No deals</p>
                  </div>
                )}
              </div>
            </div>
