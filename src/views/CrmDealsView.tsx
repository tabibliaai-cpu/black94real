...
export function CrmDealsView() {
  ...
  return (
    <div className="pb-24">
      {/* Header */}
      <div className="sticky top-0 z-10 bg-[#09080f]/90 backdrop-blur-md border-b border-white/[0.06] px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-[#f0eef6]">Deals</h1>
          </div>
          <button
            onClick={() => {
            ...
            <div key={stage} className="shrink-0 w-[260px]">
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-[13px] font-semibold text-[#f0eef6]">{stage}</h3>
              ...
              <div className="space-y-2">
                {stageDeals.map((deal) => (
                  <div key={deal.id} className="rounded-xl bg-[#110f1a] border border-white/[0.06] p-3">
                    <h4 className="text-[14px] font-semibold text-[#f0eef6] leading-snug">{deal.name}</h4>
                    ...
                    <div className="flex gap-1 mt-2.5 pt-2.5 border-t border-white/[0.04] overflow-x-auto no-scrollbar">
                      {STAGES.filter(s => s !== stage).map(s => (
                        <button
                          ...
                          className="px-2 py-0.5 rounded text-[10px] font-semibold bg-white/[0.06] text-[#f0eef6] hover:bg-white/[0.1] whitespace-nowrap transition-colors"
                        >
                          → {s}
                        </button>
                      ))}
