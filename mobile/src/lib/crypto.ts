/**
 * crypto.ts — End-to-End Encryption Module (React Native)
 *
 * Uses libsodium's X25519 for key exchange and XSalsa20-Poly1305 for
 * authenticated symmetric encryption. Same cryptographic primitives as
 * Signal Protocol, WhatsApp, and Matrix.
 *
 * Architecture:
 * ─────────────
 ...
 * 3. Private key is stored in react-native-keychain (NEVER leaves the device)
 ...
 *
 * Key storage difference from web:
 *   - Web: uses IndexedDB (via keyStore.ts)
 *   - Mobile: uses react-native-keychain (via keychain.ts)
 */

import sodium from 'libsodium-wrappers';
import { saveKeypair, getKeypair } from './keychain';
 ...
 * - If a keypair exists in keychain, return it
...
export async function getOrCreateKeyPair(userId: string): Promise<{
  ...
  if (existing) {
    const publicKey = sodium.from_base64(existing.publicKey);
    const privateKey = sodium.from_base64(existing.privateKey);
    return {
  ...
  // Persist to keychain (store as base64 strings since keychain stores strings)
  const publicKeyBase64 = sodium.to_base64(keypair.publicKey);
  const privateKeyBase64 = sodium.to_base64(keypair.privateKey);
  await saveKeypair(userId, publicKeyBase64, privateKeyBase64);
 ...
 * @param myPrivateKey - Sender's private key (Uint8Array from getOrCreateKeyPair)
 ...
 * @param myPrivateKey - Recipient's private key (Uint8Array from getOrCreateKeyPair)
...
export async function decryptMessage(
  ...
  try {
    ...
    console.warn(
      '[E2E] Decryption failed — message may be from before encryption or keys mismatch:',
      err,
    );
    return null;
  }
